...
export default function SitesPage() {
  ...
  useEffect(() => {
    ...
    // Refresh toutes les 30s si des sites sont en cours
          ...
          {sites.map((site: any) => {
            ...
            return (
              <Link key={site.id} href={`/sites/${site.id}`} style={{ textDecoration: 'none' }}>
                <div style={{
                  background: '#0D1117', border: '1px solid #1E2D3D', borderRadius: 14,
                  padding: '20px 24px', transition: 'border-color 0.15s', cursor: 'pointer',
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    {isGenerating && progress && (
                      <div style={{ marginTop: 10 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#8B9CB0', marginBottom: 4 }}>
                          <span>Génération du contenu IA</span>
                          <span>{progress} paires</span>
                        </div>
                    ...
                    {isGenerating && !progress && (
                      <div style={{ marginTop: 8, fontSize: 11, color: '#9F7AEA' }}>
                        🌙 Génération programmée cette nuit à 1h UTC
                      </div>
                    )}
                  </div>
      ...
      <div style={{ marginTop: 32, padding: 16, background: '#0D1117', border: '1px solid #1E2D3D', borderRadius: 12 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
          {Object.entries(STATUS_CONFIG).map(([key, val]) => (
            <span key={key} style={{ fontSize: 11, color: val.color }}>
              {val.icon} {val.label}
            </span>
          ))}
        </div>
